...
import type { AcademyEnrollment } from "@/types";

interface ReceiptData extends AcademyEnrollment {
  customer: { firstname: string; lastname: string; email: string };
}

export default function AcademyReceiptPage() {
  ...
  useEffect(() => {
    ...
    api
      .get<ReceiptData>(`/academy/${params.id}/receipt`)
      .then(setReceipt)
    ...
    <main className="min-h-screen bg-gray-50 py-10 px-4 print:bg-white print:py-0">
      <div className="max-w-lg mx-auto bg-white border rounded-xl shadow-sm p-8 print:shadow-none print:border-0">
        <div className="flex items-center justify-between mb-6">
          ...
          <PdfActions
            downloadPath={`/academy/${receipt.id}/receipt/pdf`}
            filename={`ArewaTecHub_Academy_Receipt_${receipt.reference ?? receipt.id}.pdf`}
          />
        </div>

        <h1 className="text-lg font-bold text-brand-dark mb-1">Digital Academy — Receipt</h1>
        ...
        <dl className="text-sm space-y-2 mb-6">
          ...
          <div className="flex justify-between"><dt className="text-brand-muted">Batch</dt><dd>{receipt.batch?.name ?? "—"}</dd></div>
